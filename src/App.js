...
// Komponent pomocniczy do przechwytywania kliknięć myszką na mapie
...
function HeatmapCard({ title = "Mapa pomiarów", androidId = null, sessionId = null }) {
  ...
  const [viewMode, setViewMode] = useState("measurements"); // poziomy: measurements, propagation, routing
  ...
  const [routeStatus, setRouteStatus] = useState(""); // Komunikat o stanie trasy
  ...
  if (!startPoint) {
    ...
    setRouteStatus("📍 Wybrano Start. Kliknij teraz punkt końcowy.");
  } else if (!endPoint) {
    const end = [latlng.lat, latlng.lng];
    setEndPoint(end);
    setRouteStatus("🔍 Algorytm analizuje bazę pomiarów w poszukiwaniu anomalii sieciowych...");

    // 1. Obliczamy punkt środkowy trasy, żeby wyznaczyć korytarz poszukiwań dziur w zasięgu
    const midLat = (startPoint[0] + end[0]) / 2;
    const midLon = (startPoint[1] + end[1]) / 2;

    // 2. Szukamy w pamięci punktu o krytycznym RSRP (< -100 dBm)
    // ZASTRZEŻENIE: Punkt musi być BLISKO naszego korytarza podróży (max ok. 1.2 km od środka)
    const maxDetourRadius = 0.012; // tolerancja w stopniach geograficznych (ok. 1.2 km we Wrocławiu)
    
    const anomalyPoint = measurements.find(m => {
      const mLat = m.latitude ?? m.location_lat;
      const mLon = m.longitude ?? m.location_lon;
      
      const distanceToMidpoint = Math.sqrt(Math.pow(mLat - midLat, 2) + Math.pow(mLon - midLon, 2));
      return m.rsrp < -100 && distanceToMidpoint < maxDetourRadius;
    });

    // 3. Budujemy adres URL dla OSRM w zależności od tego, czy znaleźliśmy anomalię w korytarzu
    let url = "";
    if (anomalyPoint) {
      const aLat = anomalyPoint.latitude ?? anomalyPoint.location_lat;
      const aLon = anomalyPoint.longitude ?? anomalyPoint.location_lon;
      
      // Zmuszamy OSRM do jazdy przez: Start -> Dziura w zasięgu -> Koniec
      ...
      setRouteStatus("🛣️ Optymalizacja: Wykryto słaby zasięg w pobliżu! Trasa została skorygowana o punkt diagnostyczny.");
    } else {
      // Brak anomalii w pobliżu lub są za daleko (zabezpieczenie przed 10km objazdem) -> jedź najkrótszą drogą
      ...
      setRouteStatus("🟢 Trasa rutynowa: Brak bliskich anomalii sieciowych. Wyznaczono najkrótszą drogę.");
    }

    // 4. Pobieramy geometrię wyznaczonej drogi
    try {
      ...
      setRouteStatus("❌ Błąd połączenia z serwerem map.");
    }
  } else {
    // Reset przy trzecim kliknięciu
    setStartPoint([latlng.lat, latlng.lng]);
    setEndPoint(null);
    setRouteCoords([]);
  ...
  return (
      <section className="map-card">
        ...
        {/* Panel informacyjny dla trybu planowania tras */}
        {viewMode === "routing" && (
          <div style={{ padding: "10px", backgroundColor: "#f0f4f8", borderRadius: "8px", marginBottom: "12px", fontSize: "13px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span>{routeStatus} {routeDistance && <strong>({routeDistance} km)</strong>}</span>
        ...
        <div className="leaflet-map-wrapper">
          <MapContainer center={FALLBACK_CENTER} zoom={12} scrollWheelZoom className="leaflet-map">
            ...
            {/* Renderowanie punktów pomiarowych */}
            ...
            {/* Renderowanie danych propagacyjnych od Madzi */}
